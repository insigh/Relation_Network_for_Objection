...
class resnet_v1_101_rcnn_attention_1024_pairwise_position_multi_head_16_learn_nms(resnet_v1_101_rcnn_learn_nms_base):
    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if is_train:
            ...
            nms_logit_weight = mx.sym.var('nms_logit_weight', shape=(len(cfg.network.NMS_TARGET_THRESH.split(',')), 128), dtype=np.float32)
            nms_logit_bias = mx.sym.var('nms_logit_bias', shape=(len(cfg.network.NMS_TARGET_THRESH.split(',')),), dtype=np.float32)
