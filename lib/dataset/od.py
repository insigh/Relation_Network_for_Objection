...
def od_results_one_category_kernel(data_pack):
...
class od(IMDB):
    def __init__(self, image_set, root_path, data_path, result_path=None, rpn_path=None, mask_size=-1, binary_thresh=None):
        ...
        """
        super(od, self).__init__('COCO', image_set, root_path, data_path, result_path, rpn_path)
        ...
        #get annotaion file to find the path of each iamge
    ...
    def image_path_from_index(self, index):
        ...
        filename = (self.image_paths[str(index)])
    ...
    def _write_coco_results(self, all_boxes, res_file, ann_type, all_masks):
        ...
        results = pool.map(od_results_one_category_kernel, data_pack)
        pool.close()
        pool.join()
        results = sum(results, [])
